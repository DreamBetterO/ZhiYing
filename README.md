...
更多使用细节见 [软件介绍](docs/项目文档/软件介绍.md)。

## 性能与消耗参考

多次实测（以 3 小时课程视频为基准）：

- **整理速度**：本地约 17 分钟完成 → 折算 **1 小时视频约 6 分钟**
- **云端 token 消耗**：合计约 13 万 → 折算 **1 小时视频约 4.3 万 tokens**（输入约 3.1 万、输出约 1.3 万）

实际耗时与消耗随视频内容、语言、模型与所选链路（本地 / 云端）浮动，以上仅作量级参考。原始状态条见 [软件介绍](docs/项目文档/软件介绍.md#任务完成)。

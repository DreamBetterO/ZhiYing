...
class SqliteCheckpointAdapter:
    def __init__(self, database: Path, graph_version: str) -> None:
        ...
        self.connection.execute("CREATE TABLE IF NOT EXISTS video_study_graph_threads (thread_id TEXT PRIMARY KEY, graph_version TEXT NOT NULL)")
        self.connection.commit()

    def config_for(self, thread_id: str) -> dict:
        row = self.connection.execute("SELECT graph_version FROM video_study_graph_threads WHERE thread_id = ?", (thread_id,)).fetchone()
        if row is not None and row[0] != self.graph_version:
            raise ValueError("GRAPH_VERSION_MISMATCH：必须创建新 thread，旧 checkpoint 不可转换")
        if row is None:
            self.connection.execute("INSERT INTO video_study_graph_threads VALUES (?, ?)", (thread_id, self.graph_version))
            self.connection.commit()

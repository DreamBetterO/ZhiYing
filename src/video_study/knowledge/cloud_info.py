...
def merge_cloud_info(
    ...
    if int(snapshot.get("requests_used", 0)) > 0:
        merged["request_budget"] = {
            **{key: snapshot[key] for key in ("max_requests", "requests_used", "requests_remaining")},
            "current_run_requests_used": int(snapshot["requests_used"]),
            "lineage_attempts": len(attempts),
        }
    return merged

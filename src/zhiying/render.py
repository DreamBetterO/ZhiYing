...
def render_docx(
    ...
    node = find_tool("node")
    renderer = project_root / "scripts" / "renderers" / "render_docx.mjs"
    modules = project_root / "node_modules" / "docx"
    if not renderer.is_file():
        renderer = project_path("scripts", "renderers", "render_docx.mjs")
...
class DocumentAdapter:
    ...
    def _legacy_render_view(self, document: dict) -> dict:
        if int(document.get("schema_version", 1) or 1) == 3:
            from .documents.v3 import v3_to_v2
...
class DocumentAdapterV31:
    ...
    - Word：scripts/renderers/render_docx_v31.mjs（公式生成 Word 原生 OMML）；
    ...
    def _figures_for(self, document_json: Path) -> dict[str, dict]:
        ...
        from .documents.render_v31 import resolve_v31_figures
    ...
    def render_markdown(
        ...
        from .documents.render_v31 import render_markdown_v31
    ...
    def render_word(self, document_json: Path, output: Path, *, cancel_check) -> Path:
        from .documents.render_v31 import render_docx_v31
    ...
    def render_pdf(
        ...
        *,
        ...
        from .documents.render_v31 import render_pdf_v31
        figures = self._figures_for(source_document) if source_document is not None else {}

...
class QwenVLSession:
    ...
    def start(self) -> None:
        if self.process is not None and self.process.poll() is None:
            return
        self._temporary = tempfile.TemporaryDirectory(prefix="zhiying-vlm-session-")
...
class LocalQwenVLProvider:
    ...
    def __init__(self, settings: dict[str, Any]) -> None:
        ...
        self.runner = _resolve_path(root, str(settings.get("local_vlm_runner", "scripts/workers/qwen_vl_runner.py")))

...
8. **开发python所在环境**：conda环境，需要使用`conda activate ImageT10`激活。非必要不要动核心依赖。
...
### Feedback
- [2026-08-16 00:06:00] 用户要求升级实施时注重"链路完整性、可用性、干净性"，不能仅命令行跑通，必须完整挂载 UI 的逻辑跑通；不硬编码、不冗余代码。**Why:** 用户关心端到端可用性而非仅 CLI 层面验证。**How to apply:** 所有升级实施后需验证 UI 管线完整挂载，代码无魔法值/冗余路径。

### Project
- [2026-08-16 10:02:24] [project] V4.2 实机复核结论：两个 ASR 模型权重完整；正式 UI 启动组合下 faster-whisper GPU/CPU 可加载，test4-2 可在 Qwen3-ASR 失败后降级到 faster-whisper GPU 并完成 15 步、VLM GPU 和 MD/Word/PDF。Qwen3-ASR 尚不可用，原因是开发 overlay 缺官方依赖、便携 runtime 构建未包含 qwen-asr，runner 又掩盖原始导入错误。当前产品仍报告 0.4.1，V4.2 不得标记完成；后续唯一入口为 `迭代升级/V4.2本地ASR实机复核与收尾修复清单.md`。PyInstaller 0.4.1 打包资源保留在 `packing/`，不能作为 Qwen3-ASR 可用的 V4.2 发行物。
